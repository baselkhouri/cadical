...
int Drup2Itp::init_data_structures () {
  ...
  if (!max_var)
    return 0;
  // The order of the below 3 methods is critical.
  init_vals ();
...
bool Drup2Itp::supported_option (const char *arg) const {
  ...
  //   return false;
  // if (starts_with_suffix (arg, "phase"))
  //   return false;
...
void Drup2Itp::add_derived_clause (uint64_t id, bool, const vector<int> &c,
  ...
  if (c.empty ()) {
    ...
    // tracer API can provide the necessary information on the top conflict
    // without antecedents. This is a temporary hack to handle the top
    // conflict. It's fragile and should be replaced with a better solution.
...
void Drup2Itp::print_stats () {
  ...
  MSG ("original core:   %15" PRId64
       ...
       stats.original, CaDiCaL::percent (stats.core, stats.original));
}
