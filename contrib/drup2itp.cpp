...
Drup2Itp::Drup2Itp ()
    : internal (0), top_conflict (0), conflict (0),
      sorter (&lit2trail, vals), current_part (0), maximal_part (1),
...
void Drup2Itp::label_root_level (ResolutionProofIterator &it,
  ...
  while (trail_label_idx < trail_sz) {
    ...
    Range &c_range = c->range, &lit_range = ranges[idx];
    lit_range = c_range;
...
bool Drup2Itp::replay (ResolutionProofIterator &it, bool undo) {
  ...
  //   failing_assumptions[0].size() > 1) {
  //     // Note proof.back()->clause() and failing_assumptions[0] are not
  //     necessarily
  //     // the same clause as proof.back()->clause() can be optimized.
  //     label_final (it, proof.back()->clause());
  //   } else
...
bool Drup2Itp::supported_option (const char *arg) const {
  if (starts_with_suffix (arg, "check"))
    return false; // we write to internal->clause_id++
